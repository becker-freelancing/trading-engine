...
import static com.becker.freelance.math.Decimal.INFLATED;
import static com.becker.freelance.math.MathConst.LONG_MASK;
...
class MutableBigInteger {
     ...
     * and scale.
     */
    Decimal toDecimal(int sign, int scale) {
        if (intLen == 0 || sign == 0)
            return Decimal.zeroValueOf(scale);
        ...
        if (len > 2 || (d < 0 && len == 2))
            return new Decimal(new BigInteger(mag, sign), INFLATED, scale, 0);
        long v = (len == 2) ?
                ((mag[1] & LONG_MASK) | (d & LONG_MASK) << 32) :
                d & LONG_MASK;
        return Decimal.valueOf(sign == -1 ? -v : v, scale);
    }
