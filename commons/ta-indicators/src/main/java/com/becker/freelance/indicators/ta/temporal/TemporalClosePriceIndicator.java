...
public class TemporalClosePriceIndicator implements TemporalIndicator<Num> {

    private final TemporalBarSeries temporalBarSeries;

    public TemporalClosePriceIndicator(TemporalBarSeries temporalBarSeries) {
        this.temporalBarSeries = temporalBarSeries;
    }

    @Override
    public Num getValue(LocalDateTime time) {
        return temporalBarSeries.getBarSeries().getBar(temporalBarSeries.mapTimeToIndex(time)).getClosePrice();
    }
