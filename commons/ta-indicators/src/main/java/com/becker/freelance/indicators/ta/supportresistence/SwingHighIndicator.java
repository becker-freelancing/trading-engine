...
public class SwingHighIndicator implements Indicator<Optional<SwingPoint>> {

    private final List<SwingPoint> indexes;
    private final Indicator<Num> sma;

    public SwingHighIndicator(Indicator<Num> source, int period) {
    ...
    @Override
    /**
     * Gibt den letzten SwingHigh-Point zurück
     */
    public Optional<SwingPoint> getValue(int index) {
        ...
        boolean low = current.isLessThan(last) && preLast.isLessThan(last);
